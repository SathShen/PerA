...
class VisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        w0, h0 = w0 + 0.1, h0 + 0.1
        is_change_dtype = False
        if patch_pos_embed.dtype == torch.bfloat16:
            patch_pos_embed = patch_pos_embed.to(dtype=torch.float32)
        ...
        if is_change_dtype:
            patch_pos_embed = patch_pos_embed.to(dtype=torch.bfloat16)
...
class DINOHead(nn.Module):
    def __init__(self, in_dim, out_dim, use_bn=False, norm_last_layer=True, nlayers=3, hidden_dim=2048, bottleneck_dim=256):
        ...
        if nlayers == 1:
            self.mlp = nn.Linear(in_dim, bottleneck_dim, bias=True)
        else:

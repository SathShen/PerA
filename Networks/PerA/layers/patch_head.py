...
class MLPHead(nn.Module):
    def __init__(self, in_dim, use_bn=False, nlayers=3, hidden_dim=2048, bottleneck_dim=256, mlp_bias=True):
        super().__init__()
...
class PerA_patch_head(nn.Module):
    def __init__(self, in_dim, out_dim, use_bn=False, nlayers=3, hidden_dim=2048, bottleneck_dim=256, mlp_bias=True):
        super().__init__()
        nlayers = max(nlayers, 1)
            
        self.mlp0 = MLPHead(in_dim, use_bn, nlayers, hidden_dim, bottleneck_dim, mlp_bias)
        self.avgpool = nn.AdaptiveAvgPool1d(bottleneck_dim)
        self.mlp1 = MLPHead(bottleneck_dim, use_bn, nlayers, hidden_dim, bottleneck_dim, mlp_bias)
